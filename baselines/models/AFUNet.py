...
def main():
    ...
    parser.add_argument("--in_chans", type=int, default=18)  # AFUNet 默认输入 18 通道
    parser.add_argument("--embed_dim", type=int, default=72)
    parser.add_argument("--depths", nargs='+', type=int, default=[5, 5, 5, 5])
    parser.add_argument("--num_heads", nargs='+', type=int, default=[4, 4, 4, 4])
    parser.add_argument("--window_size", type=int, default=8)
    parser.add_argument("--device", type=str, default="cuda")
    args = parser.parse_args()

    # 构造 dummy input: [B, C, H, W]
    device = torch.device(args.device if torch.cuda.is_available() else "cpu")
    x = torch.randn(1, 18, 1080, 1920).to(device)
    ...
    print(f"MACs: {macs / 1e9:.3f} G ({macs / 1e12:.3f} T)")
