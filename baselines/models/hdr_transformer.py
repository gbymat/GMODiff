...
class HDRTransformer(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        # x: 输入为3帧LDR图像 [B, 3, 3, H, W] -> [暗帧, 参考帧, 亮帧]
        dark, ref, bright = x1, x2, x3
        h, w = ref.size()[-2:]

        # 统一padding到8的倍数
        paddingBottom = int(np.ceil(h / 8) * 8 - h)
        paddingRight = int(np.ceil(w / 8) * 8 - w)
        pad = nn.ReplicationPad2d((0, paddingRight, 0, paddingBottom))
        x1, x2, x3 = pad(dark), pad(ref), pad(bright)
...
def test_flops():
    ...
    x1 = torch.randn((1, 6, height, width)).cuda()
    x2 = torch.randn((1, 6, height, width)).cuda()
    x3 = torch.randn((1, 6, height, width)).cuda()
    model = HDRTransformer(img_size=(128, 128),
                           window_size=window_size, img_range=1., depths=[6, 6, 6],
                           embed_dim=60, num_heads=[6, 6, 6], mlp_ratio=2, in_chans=6).cuda()
    model.eval()
